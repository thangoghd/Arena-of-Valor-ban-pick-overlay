    ...
    <Container>
      ...
      <PicksContainer id="picks_blue" team="blue">
        {currentPicks.blue.map((pick, idx) => {
          return (
            <PickComponent phase={(currentSlot >=4 && currentSlot <= 9) ? 'three-big-two-small' : currentSlot >= 14 && currentSlot <= 17 ? 'three-small-two-big' : ''} team={"blue"} idx={idx} slot={pick.id} key={pick.id} champion={pick.champion} playerIGN={playerIGNs[idx].ign} currentSlot={currentSlot}/>
          )
        })}
      </PicksContainer>
      <PicksContainer id="picks_red" team="red">
        {currentPicks.red.map((pick, idx) => {
          return (
            <PickComponent phase={(currentSlot >=4 && currentSlot <= 6) ? 'three-big-two-small' : currentSlot >= 14 && currentSlot <= 17 ? 'three-small-two-big' : ''} team={"red"} idx={idx} slot={pick.id} key={pick.id} champion={pick.champion} playerIGN={playerIGNs[idx+5].ign} currentSlot={currentSlot}/>
          )
        })}
      </PicksContainer>
